...
ASP.NET 核心 Microsoft 重新设计的 ASP.NET 是服务器端 web 应用开发框架，可帮助您有效地构建 web 应用。它运行在.NET 核心平台之上，使您的应用能够在各种平台上运行，包括 Linux 和 macOS。这打开了大量的机会，在这个时代成为一名.NET 开发人员是令人兴奋的。
...
*   掌握 ASP.NET MVC 的理念
*   创建第一个 ASP.NET 核心 Web 应用项目和项目结构

# Web 应用简介

在讨论 ASP.NET 核心及其特性之前，让我们先了解 web 应用开发的基础知识。
...
*   **ASP.NET 核心**：全新的 ASP.NET 平台，跨平台运行。各小节包括：
    ...
    *   **Web 应用**（Razor Pages）：Razor Pages 是 ASP.NET 核心 MVC 的一项功能，它使编写以页面为中心的场景更容易、更高效。
...
无论何时在文件系统（ASP.NET 核心项目文件夹内）中添加文件或文件夹，更改都会自动反映在应用中。
...
| 其他文件 | `appsettings.json`文件是配置文件，您可以在其中配置应用级设置。以前，`.xml`文件用于配置；但是，`.json`格式不太详细，**Bower**和**npm**（**节点包管理器**是客户端技术，由 ASP.NET 核心应用支持。`Bundle.config`文件允许我们配置如何将 CSS 和 JS 文件捆绑到一个文件中。 |
...
2.  选择 ASP.NET 核心 Web 应用。（可选）为项目指定名称，或接受默认名称。然后，单击 OK。
3.  请确保选择.NET Core 2.0。如果没有显示，请从[下载.NET 核心 SDKhttps://www.microsoft.com/net/download/](https://www.microsoft.com/net/download/) 核心并重新启动 Visual Studio。然后选择 Web 应用并单击 OK。
4.  在项目上单击鼠标右键，然后单击“构建”。这将恢复依赖项。

# 创建第一个应用

现在是创建第一个 ASP.NET 核心应用的时候了。

启动 Visual Studio 并执行以下步骤：

1.  通过在 Visual Studio 中选择文件|新建项目来创建项目。第一个选项用于创建 ASP.NET web 应用的早期版本。第二个选项是使用.NET 核心框架创建 ASP.NET 核心应用。NET 核心仅支持核心功能。使用.NET 核心库的优点是它可以部署在任何平台上。选择 ASP.NET 核心 Web 应用：
...
2.  从 ASP.NET 核心模板列表中选择**空**模板。第二个选项用于创建 Web API 应用（用于构建基于 HTTP 的服务），第三个选项用于创建包含一些基本功能的 Web 应用，这些功能可以在开箱即用的情况下运行，而无需编写任何内容：
...
ASP.NET 核心运行时通过 main 方法调用`ConfigureServices`和`Configure`方法。例如，如果要配置任何服务，可以在此处添加它。您的应用的任何自定义配置都可以添加到此`Configure`方法中：
