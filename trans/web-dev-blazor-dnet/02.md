...
`Program.cs`是被调用的第一个类。它也不同于Blazor服务器和Blazor网络组件。
...
Blazor尔服务器项目有一个位于`pages`文件夹中的`_Host.cshtml`文件。这是一个 Razor 页面，它与 Razor 组件不是一回事:
...
然后我们有一些标准的 HTML 文档类型、元标签、标题和样式。Blazor特有的唯一东西是`base`标签:

```cs
<base href="~/" />
```

它确保你的页面会找到Blazor信号中枢。如果您没有`base`标签，一旦您导航到文件夹中的页面，您的网站将会崩溃，因为相对网址不再找到 Blazor 信号中心。
