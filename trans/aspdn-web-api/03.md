...
您还可以在应用程序的`web.config`配置文件中为您的 WCF 4.5 服务定义虚拟服务激活端点。 这样，您就可以在不需要任何`.svc`文件的情况下激活 WCF 服务。 下面是您需要在应用程序的`web.config`配置文件中指定的配置，以激活您的服务，而不需要`.svc`文件:
...
因此，你应该在你的应用程序的`web.config`文件中设置缓存配置文件，如下代码所示:
