...
本章讨论我们可以应用于 ASP.NET 核心应用的不同优化，以便它们执行得更快，能够处理更多的并发连接。我们将关注的性能和可伸缩性这两个概念是不同的，事实上，它们在某种程度上相互冲突。您必须应用正确的优化级别才能找到最佳点。
...
托管是用于运行 ASP.NET 核心应用的进程。在 ASP.NET Core 中，您有两种现成的托管选择：
