...
阅读本章后，您将对围绕 ASP.NET 核心应用的安全性的许多方面有很好的理解。
...
从 3.x 版开始，默认的 ASP.NET 核心模板包括获得用户对使用 cookie 的批准的支持。用于提供诸如到期等 cookie 数据的`CookieOptions`类现在有了一个新属性`IsEssential`，这取决于应用的 cookie 策略，由其`CookiePolicy`实例的`CheckConsentNeeded`属性决定。这实际上是一个函数，如果它返回`true`但用户没有明确授予权限，则某些事情将无法工作：`TempData`和`Session`cookies 将无法工作。
