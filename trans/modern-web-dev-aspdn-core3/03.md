...
在旧的 web 应用时代，事情很简单，如果你想要一个页面，你必须有一个物理页面。然而，事情已经发生了变化，ASP.NET 核心现在是一个 MVC 框架。这是什么意思？嗯，在 MVC 中，没有物理页面（尽管这并不完全正确）；相反，它使用路由将**请求**定向到**路由****处理程序**。MVC 中最常见的路由处理程序是**控制器动作**。在本章之后，您将学习如何使用路由访问控制器操作。
...
为了让 ASP.NET Core服务物理文件，需要对此进行配置，我们使用`Configure`中的`UseStaticFiles`扩展方法，将静态文件、处理中间件添加到管道中；对`UseStaticFiles`的调用包含在 ASP.NET 核心 web 应用的 Visual Studio 模板中。如果我们不启用静态文件服务，或者如果不存在文件，则需要由**路由处理程序**处理请求。MVC 中最常见的路由处理程序是**控制器****动作**。
