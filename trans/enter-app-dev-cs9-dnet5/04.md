...
在前面的讨论中，因为我们有端到端的应用程序代码，所以更容易找到死锁的原因。但是，如果我们正在开发一个可以在 WPF、ASP.NET Core 5 或。NET Framework 应用程序，我们需要确保库内的异步代码不会导致死锁，即使调用方可能正在通过同步方法(`GetAwaiter().GetResult()`)使用库方法。
