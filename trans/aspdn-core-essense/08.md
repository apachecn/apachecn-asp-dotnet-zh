...
下面是对如何在新的 ASP 中启用身份验证的高级概述.NET Core应用使用 web 模板:
...
下面是对如何在 ASP 中实现基本授权技术的高级概述.NET Core应用:
...
您可能想要将这些数据保护代码添加到您的 ASP.NET Core控制器在您的 web 项目。 相反，您应该考虑抽象服务类来处理应用的特定部分，以保持控制器的精简。
