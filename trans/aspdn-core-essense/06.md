...
值得注意的是，EF Core 可以与不止 ASP 一起使用.NET Core和关系数据库。 事实上,它也被设计用于与**环球 Windows 平台**(**UWP)应用在 Windows 上 10 和 Windows 桌面应用(**Windows Presentation Foundation (WPF**)或【显示】WinForms)。 除了 SQL Server, EF Core 还可以与**Azure Table Storage**，**PostgreSQL**，**SQLite**，甚至**NoSQL**数据库一起使用。******
