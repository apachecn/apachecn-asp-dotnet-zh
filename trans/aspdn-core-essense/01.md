...
本书将主要关注 Visual Studio 2015。 所有的截图和说明都将使用 Visual Studio 2015 Community Edition 和运行在 Windows 10 上的 Edge 网络浏览器。 如果使用不同的编辑器、操作系统或 web 浏览器，则必须在自己的环境中执行相同的步骤。 无论使用什么平台，应用程序代码都应该保持不变。
