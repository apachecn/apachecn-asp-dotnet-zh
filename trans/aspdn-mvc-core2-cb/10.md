...
我们已经讨论了用 ASP 注入依赖关系.NET Core[第五章](05.html)，*SOLID 原理，反转控制，依赖注入*。 我们了解到 IoC 机制是 ASP 的内部机制.NET的核心。 这是由构造函数完成的，它的生命周期必须在`Startup.cs`中的`Configure`方法中配置。 我们会做一些调整，一切都会自动工作。
...
在本食谱中，您将学习如何在 ASP 中管理区域.NET的核心。 为此，我们将:
...
我们添加了 MVC、实体框架、Automapper 和 MediatR 作为 ASP 的服务.NET Core应用。 我们现在可以使用它们。
