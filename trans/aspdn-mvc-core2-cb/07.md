# 使用 Micro ORMs、NoSQL 和 Azure 访问数据
...
16.  让我们打开 Visual Studio 并创建一个新的空 ASP.NET 核心应用(.NET 框架):
...
16.  现在我们有了要从 Azure 存储中检索的数据，让我们创建一个空的 ASP.NET 核心应用与。NET 框架。 我们在文件夹中创建我们需要的结构:`Models`、`Repository`、`Controllers`和`Views`。
