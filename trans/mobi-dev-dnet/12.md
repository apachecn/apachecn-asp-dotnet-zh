...
大多数移动应用都与某些数据集耦合，这些数据集要么通过服务后端下载，要么绑定到应用中。 这些数据集可以是不同的，从简单的静态文本内容到与特定上下文相关的事务集的实时更新。 在这种情况下，开发人员的任务是在远程交互和数据缓存之间创建最佳平衡。 此外，离线支持正在成为移动应用开发领域的规范。 为了避免数据冲突和同步问题，开发人员必须认真考虑根据手头数据类型实现的过程。 在本章中，我们将讨论可能使用 SQLite 和 Akavache 的数据同步和离线存储场景，以及新的。net 核心模块，如实体框架核心。
