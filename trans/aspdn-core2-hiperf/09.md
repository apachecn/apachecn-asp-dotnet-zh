...
ASP.NET 核心 2.0 现在支持在 Windows 上使用 IIS 时从 Visual Studio 2017 进行调试。您可以附加到流程中，并像在传统的 ASP.NET 项目中一样逐步执行代码。如果您计划在生产中使用 IIS 的完整版本，那么在该版本上进行测试是一个好主意。用红隼或 IIS Express 进行测试只能进行到此为止，您应该尽早将目标定为尽可能像真人一样。
