...
照片图库是大多数社交媒体网站上常见的功能。 使用 jQuery 构建这些库的方法有很多种。 在本食谱中，让我们使用z-index CSS 属性来构建这样一个应用。 下表显示了本例中使用的构造的摘要:
...
5 星评级控件是一个有用的功能，当您需要评论一个项目时，比如一本书、电影、产品等等。 在本例中，让我们使用 jQuery 创建这个应用作为一个 ASP.NET用户控件。 本例中使用的结构如下:
