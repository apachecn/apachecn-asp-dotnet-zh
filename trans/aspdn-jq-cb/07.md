...
**Web API**(**Web 应用编程接口**)是使用. net 框架创建的 HTTP API。 它使用 HTTP 协议返回数据。 由于HTTP 可以在广泛的平台上使用，所以这些api 可以在 Web 和移动平台上使用，也可以跨各种设备使用。
...
Ajax调用也可以对 ASP 中的`Controller`动作进行。 净 MVC 应用。 在这个例子中，让我们从一个视图向控制器动作发送一个请求。 本例中使用的结构总结如下表:
