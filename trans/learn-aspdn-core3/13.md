...
5.  将 Tic Tac Toe web 应用发布到 Azure 应用服务。如果你不知道怎么做，你可以在[第 12 章](12.html)、*托管 ASP.NET 核心 3 应用*中查找。
...
1.  将 Tic Tac Toe Web 应用发布到 AWS Elastic Beanstalk。如果您不知道怎么做，可以在[第 12 章](12.html)、*托管 ASP.NET 核心 3 应用*中查找。
...
这对于.NET 核心代码非常有效，但是目前，您无法看到应用的 JavaScript 部分中是否出现任何错误。由于现代应用包含大量 JavaScript 代码，如果您也能够监控这些部分，那就太好了；正当好吧，你可以做到；你只需要稍微修改一下代码。
...
伟大的您可以不断监视整个应用，无论是在 JavaScript 端还是在.NET 核心端，这在出现错误行为时都非常有用。
...
6.  将 Tic Tac Toe web 应用发布到 AWS Elastic Beanstalk，以便应用新的 AWS CloudWatch 配置。如果您不知道怎么做，可以在[第 12 章](12.html)、*托管 ASP.NET 核心 3 应用*中查找。
...
在本章中，我们讨论了如何管理和监督 ASP.NET 核心 web 应用，以帮助 IT 运营部门更好地了解运行时、错误发生之前和之后发生的情况。
...
您已配置 Azure Application Insights 以实时监视 ASP.NET 核心 web 应用。你甚至能够理解并解决`404 Not Found`问题背后的奥秘。
