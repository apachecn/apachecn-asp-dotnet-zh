...
让我们从 Tic Tac Toe 演示应用中休息一下，使用服务器端 BLAZOR 模板创建一个简单的 Web 应用。首先使用 ASP.NET 核心应用框架，然后使用 Blazor 模板创建一个新项目，如下所示：

![](img/8dc1e7a0-b27b-4a0d-a739-5b52c57be9f3.png)

创建 web 应用时，您会立即注意到它的项目结构通常类似于任何 ASP.NET 核心模板，具有以下`Startup`和`Program`类：
