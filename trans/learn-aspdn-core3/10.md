...
3.  更新`UserModel`使用内置 ASP.NET Core身份认证功能，删除`IdentityUser`类已经提供的`Id`和`Email`属性：
...
9.  在`Models`文件夹中添加一个名为`RoleModel`的新类，并使其从`IdentityRole<long>`继承，因为内置 ASP.NET Core身份验证功能将使用该类：
...
13.  更新通信中间件中的两个`ProcessEmailConfirmation`方法，因为它们必须是异步的，才能使用 ASP.NET Core标识。停止使用私人定义的`private readonly IUserService _userService;`用户服务，在以下两种方法中，优先使用本地定义的用户服务：
...
24.  更新`GameInvitationController`中的`GameInvitationConfirmation`方法，使其异步。要使用 ASP.NET Core标识，控制器操作必须是异步的：
...
25.  更新`HomeController`中的`Index`和`SetCulture`方法，使其异步，以便使用 ASP.NET Core标识：
...
26.  更新`UserRegistrationController`中的`Index`方法，使其与 ASP.NET Core标识异步工作：
...
到目前为止做得很好，因为我们的应用现在已经可以使用 ASP.NET Core标识，并且在完成了前面部分中的所有准备工作之后，一般来说，它现在可以处理不同类型的身份验证。现在，我们可以开始学习如何添加不同类型的身份验证，我们将在下一节中开始学习基本的用户表单身份验证。
