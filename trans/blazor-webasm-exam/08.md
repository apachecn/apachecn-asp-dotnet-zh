...
图 8.1–Blazor WebAssembly应用项目模板
...
3.  In the **Search for templates** (*Alt*+*S*) textbox, enter `Blazor` and then hit the *Enter* key.
    ...
    图 8.4–Blazor WebAssembly应用项目模板
...
之后，我们使用微软的 **Blazor WebAssembly应用**项目模板创建了一个托管的 Blazor WebAssembly应用。我们检查了演示项目，然后将其从多项目解决方案中删除。我们增加了一个`TaskItem`类和一个`TaskItem` API 控制器。接下来，我们通过更新数据库的连接字符串并使用实体框架迁移来配置 SQL Server。最后，我们使用`HttpClient`服务来读取任务列表、更新任务、删除任务和添加新任务。
