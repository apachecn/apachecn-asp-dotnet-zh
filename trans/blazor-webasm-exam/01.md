...
在这种托管模式下，web 应用程序在服务器上执行，只有对 UI 的更新才会发送到客户端的浏览器。浏览器被视为瘦客户端，所有处理都在服务器上进行。当使用 Blazor 服务器时，用户界面更新、事件处理和 JavaScript 调用都通过 ASP.NET Core**信号器**连接来处理。
...
最后，一个 Blazor Server 网络应用程序必须由一个**ASP.NET Core**服务器提供。
