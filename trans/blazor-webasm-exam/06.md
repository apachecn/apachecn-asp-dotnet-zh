...
将使用**空 Blazor WebAssembly应用**项目模板创建`ShoppingCart`项目。首先，我们将添加逻辑来添加和移除购物车中的产品。然后，我们将演示当我们在页面之间导航时，购物车的状态会丢失。为了维护购物车的状态，我们将在 DI 容器中注册一个使用 **AppState** 模式的服务。最后，我们将演示通过将新服务注入相关组件，购物车的状态不会丢失。
...
3.  In the **Search for templates** (*Alt* + *S*) textbox, enter `Blazor` and then hit the *Enter* key.
    ...
    图 6.2–空 Blazor WebAssembly应用项目模板
...
现在，您应该能够使用 DI 将**应用状态**模式应用到 Blazor WebAssembly应用中。
