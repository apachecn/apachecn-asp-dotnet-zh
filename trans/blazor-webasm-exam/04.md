...
将使用**空 Blazor 网络组装应用**项目模板创建`LocalStorage`项目。首先，我们将添加一个带有 JavaScript 函数的 JavaScript 文件，我们的服务将需要使用这些函数来更新浏览器的 localStorage。接下来，我们将使用。NET 方法调用 JavaScript 函数。最后，我们将测试我们的服务。
