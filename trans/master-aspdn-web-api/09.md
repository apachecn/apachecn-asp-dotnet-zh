...
数据是软件应用中的王者，无论是以数据库、文件、流等形式。很难找到不与数据库交互的应用。在上一章中，您学习了大量关于 ASP.NET 核心以路由、中间件和过滤器的形式处理请求及其安全机制的知识；它们构成了应用的匿名前端。

到目前为止，我们还没有讨论过 ASP.NET 核心处理后端（数据存储的流行术语），也就是数据库。任何 ASP.NET 核心（Web API）都必须在应用开发期间或从一开始就与数据库集成。
...
EF 6.1 是一个使用完整的.NET 框架构建的 ORM，这意味着它只适用于完整的.NET 应用。为了实现这一点，我们需要为完整的.NET 框架而不是.NET 核心创建 ASP.NET 核心。
...
如前一节所述，EF6 只在完整的.NET 框架上工作；因此，我们不能在 ASP.NET 核心应用中直接使用它们。为此，我们需要一个类库，并对现有数据库执行反向工程，AdventureWorks2014 就是我们的示例。

**逆向工程**是实体框架（ORM）中的一个过程，用于生成类/模型（它们对应于表）并构建数据库上下文以使用数据库。

创建一个空白的 Visual Studio 解决方案`AdvWorksAPI`，并向其添加类库`AdvWorksAPI.DataAccess`。这将作为数据访问层，并将在 ASP.NET 核心应用中引用。
...
# 为完整的.NET Framework 创建 ASP.NET 核心应用
...
在空白解决方案中，创建带有 ASP.NET API 模板的 ASP.NET 核心 Web 应用（.NETFramework）。这将是我们与 AdventureWorks2014 数据库集成的 ASP.NET 核心 Web API。
...
让我们使用 Dapper ORM 创建一个与 AdventureWorks2014 数据库集成的 ASP.NET 核心 Web API 应用。
...
**实体框架核心**（**EF 核心**）是微软针对.NET 核心框架的最新 ORM，符合 ASP.NET 核心路线图。现在，ASP.NET Core 和 EF Core 为构建跨平台 web 应用提供了一个很好的平台。
...
在本章中，您学习了大量有关使用 ORM（如 EF 6.x、Dapper 和 EF Core）将 ASP.NET 核心应用与数据库集成的知识。由于有许多使用数据库提供者的选项，它当然提供了很大的灵活性。
