...
当我们在[第 3 章](03.html#20R681-b5b28eac0b2e417189a3e09b61402d4f)*ASP.NET 核心 Web API 剖析*中创建一个基本的 ASP.NET 核心 Web API 并运行该应用时，我们在浏览器中看到 URL 为`http://localhost:5000/api/values`——它显示了来自值控制器的 JSON 响应。
...
将传入 HTTP 请求映射到其相应控制器的操作方法的机制称为路由。它是 ASP.NET 核心 MVC/Web API 的关键组件之一。如果没有路由机制，ASP.NET（MVC/Web API）将无法运行应用和服务请求。路由解决了上述所有问题。
...
这是 ASP.NET 核心中最基本的路由示例。由于管道仅对其进行路由，因此使用任何 URL 访问应用都将给出相同的响应。
...
当我们创建一个 ASP.NET 核心应用作为 web API 时，需要了解一些与路由相关的功能。
...
`UseMvc`中的`AttributeRouting.CreateAttributeMegaRoute`在 ASP.NET 核心 MVC 和 web API 应用中默认提供属性路由。
...
在 ASP.NET 核心应用中，默认情况下提供基于属性的路由。在`Startup`类的`Configure`方法中，`app.UseMvc()`行表示在请求处理管道中包含 MVC 中间件。
...
ASP.NET 核心作为 MVC 或 web API 应用，支持属性和集中式路由机制。可以使用 Route 属性直接在控制器和操作上设置路由，也可以通过在一个位置创建和映射所有路由来设置路由。
...
ASP.NET 核心提供了`UrlHelper`类，这是`IUrlHelper`接口的一个实现，用于为 ASP.NET 核心应用（MVC 或 web API）构建 URL。方法`CreatedAtRoute`和`CreatedAtAction`是链接生成的两种内置方法。让我们在一个例子中使用它们：
...
NET 核心以中间件、基于约定的路由、直接或属性路由、约束等形式提供了一种轻量级、完全可配置的路由机制。正确使用这些路由功能对于应用的最佳性能是必要的。
...
下一章将重点介绍 ASP.NET 核心的中间件概念——您将学习中间件的基础知识、编写一些自定义中间件以及 ASP.NET 核心应用中的过滤器。