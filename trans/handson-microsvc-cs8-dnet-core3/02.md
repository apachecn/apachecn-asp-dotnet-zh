...
此修复程序随 Visual Studio Preview 一起提供(在撰写本书时)；您至少需要 VS16.5 才能查看 WinForms 或 WPF 的设计器。您可以通过安装 VS16.5 进行检查。要确保您从 Visual Studio 选项中标记了预览 SDK，请转到工具|选项|环境|预览功能中的设置，然后确保您选中了使用预览窗口窗体设计器进行.NET Core应用，如下图所示:
...
3.  从可用的模板选项中，选择 ASP.NET Core网络应用，然后单击下一步。以下屏幕截图显示了“创建新项目”窗口:
