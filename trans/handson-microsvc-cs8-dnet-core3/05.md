...
*   **中央数据库作为单点故障** : 在单体应用程序中，大型集中式数据库是单点故障。这个数据库通常很大，很难分解。这导致**平均恢复时间** ( **MTTR** )和**平均故障间隔时间** ( **平均故障间隔时间**)增加。
...
2.  在新 ASP.NET Core网络应用程序窗口中，选择。网络核心和 ASP.NET Core 3.1:

![](img/40e6e5cb-9888-4a02-845e-09dae9754aca.png)

3.  从创建新的 ASP.NET Core网络应用程序窗口中，选择。NET Core和 ASP.NET Core 3.1。从可用模板中选择网络应用程序(模型-视图-控制器):
