...
在微服务安全方面，当与 ASP.NET Core合作时，我们可以使用不同的安全机制，如 OAuth2.0、JWT 和 Azure Active Directory(带有额外的授权实现)。在微服务的情况下，并不是每个服务都必须进行身份验证，有些服务不需要任何身份验证来返回响应。总的来说，现在安全是一个大问题。
...
**JSON 网络代币** ( **JWT** )发音为 *JOT* 。这是一个定义良好的 JSON 模式或格式，用于描述数据交换过程中涉及的令牌。JSON 网络令牌在[https://tools.ietf.org/html/rfc7519](https://tools.ietf.org/html/rfc7519)详细定义。JWTs 可以用来验证请求，而不需要使用 OpenID 和 OAuth2.0。NET Core应用程序。
