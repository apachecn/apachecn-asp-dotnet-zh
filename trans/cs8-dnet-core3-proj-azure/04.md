...
使用 Visual Studio 2019，创建 ASP.NET Core网络应用程序:

1.  创建新的 ASP.NET Core网络应用程序:
...
我们可以使用一些 ASP.NET Core代码生成工具来帮助我们搭建脚手架，而不是自己编写所有的代码。
...
控制器是构建 ASP.NET Core MVC 应用程序的基本构件。控制器内部的方法被称为动作。因此，我们可以说控制器定义了一组动作。操作处理请求，这些请求通过路由映射到特定的操作。
