...
由于我们的要求之一是，该解决方案应尽可能在没有连接的情况下工作，如果没有连接，只需连接到本地网络，我们的项目将由本地托管的 ASP.NET Core应用编程接口和 UWP 应用组成。每个需要访问该应用的人都将获得一个平板电脑，他们可以在其中检查或更新库存。我们系统的架构看起来像这样(不完全是原始架构):
...
让我们从创建我们的身份服务器开始，它可以是一个标准的 ASP.NET Core网络应用:
