...
ASP.NET Core 2.0 应用既可以运行在。NET Core Framework 上，也可以运行在完整的。NET Framework 上。 ASP.NET 核心框架的架构是为了为应用提供一个优化的开发框架，这些应用必须部署在云或本地。 它由开销最小的模块化组件组成，因此在构思和实现您的软件解决方案时，您可以保持高度的灵活性。 您可以开发和运行您的 ASP。 Windows、Linux 和 macOS 上的 NET Core 2.0 应用。
