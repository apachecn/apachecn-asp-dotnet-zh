...
`Program`类是 ASP 的主要入口.NET Core 2.0 应用。 事实上,ASP。 在这方面，NET Core 2.0 应用非常类似于标准的。NET Framework 控制台应用。 两者都有一个在运行应用时执行的`Main`方法。 即使是接受字符串数组作为参数的`Main`方法的基本签名也是一样的，如下面的代码中所示。 毫无疑问，这是由于 ASP.NET 核心应用实际上是一个托管 web 应用的控制台应用:
