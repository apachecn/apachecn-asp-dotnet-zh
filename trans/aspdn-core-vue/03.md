...
您将在这里创建的项目还不是真实的后端 ASP.NET 核心应用，你将与 Vue.js 连接。 这样做的目的是查看在新创建的 ASP 中有哪些默认文件和文件夹。 网的核心项目。
...
ASP.NET Core 5 已经正式包含了`Swashbuckle`包，它将 Swagger 集成到的 ASP. NET Core 5 中。 `Microsoft.Extensions.DependencyInjection`命名空间中的 NET 核心 Web API。 在搭建应用时，Swagger 会通过。net CLI 自动为你安装。你可以在`Web.csproj`的`ItemGroup`中看到它:
