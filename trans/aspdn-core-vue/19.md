...
因此，GitHub Actions 是快速设置 CI/CD 和 GitHub 存储库的一个很好的解决方案，用于构建和部署。NET Core应用。 GitHub 提供了工作流、GitHub 门户内的运行器和可查看的日志，让我们的生活更轻松。 每当我们想要部署 ASP 时，我们都有大量可用的自定义工作流.NET Core应用到 Azure 或其他类型的应用到其他已知的云服务。
...
该课程已经教你如何构建一个 ASP.NET Core应用和 Vue.js 应用，像一个高级开发人员，把的价值给你的客户或客户。
