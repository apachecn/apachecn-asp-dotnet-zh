# *第四章*:在 ASP 中应用干净的架构 NET Core解决方案

您还记得前面的章节，在这一章中，您将把干净的体系结构应用到 ASP.NET Core 5 解决方案。 本章教你真实世界中文件、文件夹、项目和 ASP 的组织.NET 核心应用依赖，为你未来的大的和可扩展的 ASP.NET Core 5 企业应用。
...
**表示层**是构建 web应用的地方。 web 应用可以使用 ASP。 asp.net 核心 MVC NET Core Web API，一个**单页****应用**(**SPA**)，或者一个移动应用。
