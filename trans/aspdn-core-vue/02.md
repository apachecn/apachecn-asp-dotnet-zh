...
. net Core SDK由一些库和工具组成，这些库和工具可以让你编写。net Core 应用。 如果您正在使用 Visual Studio 2019 和 Visual Studio Mac,你不需要安装 5 . net 运行时从 Visual Studio IDE 中包括. net 核心 SDK 安装。net 核心 SDK 也有。net 命令行界面和. net 核心运行时(底层层为。net 应用运行)运行. net 核心应用。
