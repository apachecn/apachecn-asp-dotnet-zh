...
你将从设置 web 应用的后端开始，在干净的架构、**命令查询职责隔离**(**CQRS**)、中介模式和实体框架 Core 5 的指导下。 然后，本书向您展示了如何使用最佳实践构建前端应用，使用 Vuex 进行状态管理，使用 Vuetify UI 组件库，使用 vueidate 进行输入验证，使用 Vue Router 进行延迟加载，以及使用 JWT 身份验证。 稍后，您将重点关注测试和部署，在 ASP 中执行负载测试等任务.NET Core 5 和部署容器应用到云。 本书中的所有教程都支持 Windows 10、macOS 和 Linux 用户。
