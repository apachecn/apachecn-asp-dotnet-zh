...
为了加强您在这里的学习，并在继续到前端部分之前，我希望您创建一个 ASP.NET Core 5 应用。 应用应该使用你在这本书中学到的所有东西，比如干净的架构、CQRS、API 版本控制、OpenAPI 和分布式缓存，无需身份验证或使用身份验证，或者使用像 Auth0 这样的身份作为服务来节省你的时间。 我现在能想到的一个应用是电子游戏的在线商店。 实体可以是`Developer`、`Game`、`GameReviews`、`Genre`、`Publisher`等。 这个练习很简单，你可以在一周内完成。 我知道你能做到。 好运！
