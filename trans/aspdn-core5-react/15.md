...
5.  Azure DevOps将检查存储库中的代码，以便为正在使用的技术建议合适的 CI 模板。选择**ASP.NET Core**模板。不要选择**ASP.NET Core（.NET Framework）**模板。您可能需要单击**显示更多**按钮来查找**ASP.NET Core**模板。
...
*   `backend`：这将包含我们的.NET Core后端，它将用于暂存和生产环境。
