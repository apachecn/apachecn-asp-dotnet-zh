...
![Figure 11.1 – OIDC authentication flow ](image/Figure_11.01_B16379.jpg)
...
![Figure 11.2 – JWT in jwt.io ](image/Figure_11.02_B16379.jpg)
...
2.  Once we have an Auth0 account and have logged in, we need to change the default audience in our tenant settings. To get to your tenant settings, click on the user avatar and choose **Settings**:
    ...
    ![Figure 11.3 – Auth0 tenant settings option ](image/Figure_11.03_B16379.jpg)

    图 11.3–Auth0 租户设置选项

    **默认受众**选项位于**API 授权设置**部分。将此更改为`https://qanda`：

    ![Figure 11.4 – Auth0 Default Audience setting ](image/Figure_11.04_B16379.jpg)
...
4.  Select the **Single Page Web Applications** application type and click the **CREATE** button:
    ...
    ![Figure 11.5 – Creating a SPA Auth0 client ](image/Figure_11.05_B16379.jpg)
...
![Figure 11.6 – Creating an API Auth0 client ](image/Figure_11.06_B16379.jpg)
...
![Figure 11.7 – Auth0 user avatar ](image/Figure_11.07_B16379.jpg)
...
4.  Open Postman now and try to post a question:
    ...
    ![Figure 11.8 – Accessing a protected endpoint in Postman without being authenticated ](image/Figure_11.08_B16379.jpg)
...
7.  Click the **COPY TOKEN** option to copy the access token to the clipboard:
    ...
    ![Figure 11.9 – Getting a test token from Auth0 ](image/Figure_11.09_B16379.jpg)
...
8.  Back in Postman, we need to add this token to an `Authorization` HTTP header after the `bearer` word and a space:
    ...
    ![Figure 11.10 – Adding the Auth0 bearer token to an Authorization HTTP header in Postman ](image/Figure_11.10_B16379.jpg)

    图 11.10–将 Auth0 承载令牌添加到 Postman 中的授权 HTTP 头中

9.  If we send the request, it will now be successful:

    ![Figure 11.11 – Successfully accessing a protected endpoint in Postman ](image/Figure_11.11_B16379.jpg)
...
3.  Enter the path to the `questions` endpoint and press the **Ajax request** button. We see that the request is unsuccessful:
    ...
    ![Figure 11.12 – CORS error when accessing the REST API from the browser ](image/Figure_11.12_B16379.jpg)
...
8.  In the browser app, press the **Ajax request** button again. We see that the request is successful this time:
    ...
    ![Figure 11.13 – Successful cross-origin request ](image/Figure_11.13_B16379.jpg)
