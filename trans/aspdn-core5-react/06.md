...
6.  Create a `submitForm` function just above the component's `return` statement, as follows:
    ...
    因此，`question!.questionId`中的`!`阻止TypeScript抱怨`question`可能是`null`。
...
*   **React形式**：[https://reactjs.org/docs/forms.html](https://reactjs.org/docs/forms.html )
*   **React钩形式**：[https://react-hook-form.com/](https://react-hook-form.com/)