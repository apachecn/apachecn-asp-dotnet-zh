...
是时候把我们的注意力转向反应前端了。在本节中，我们将检查前端代码，从入口点开始，入口点是一个 HTML 页面。我们将探讨如何在开发模式下执行前端，以及如何构建前端以准备部署。然后，我们将了解如何管理前端依赖项，并了解为什么第一次运行应用需要一分钟的时间。最后，我们将探讨 React 组件如何组合在一起，以及它们如何访问 ASP.NET Core后端。
