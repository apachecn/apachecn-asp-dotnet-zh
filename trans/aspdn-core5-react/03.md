...
1.  We need to add the content to the `Question` component, so add the following code beneath the question title in the JSX:
    ...
    JavaScript 模板文本是 backticks（```中包含的字符串。模板文本可以包含将数据注入字符串的表达式。表达式包含在美元符号后的花括号中。这通常被称为*插值*。更多信息请访问[https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Template_literals](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Template_literals) 。
...
![Figure 3.17 – Click event ](image/Figure_3.17_B16379.jpg)
