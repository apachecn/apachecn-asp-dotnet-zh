...
集成Windows 认证是一种基于 SPNEGO、Kerberos 和 NTLMSSP 协议的认证机制。 它处理 IIS、IE 和活动目录之间的连接的自动认证。
...
让我们实现对 ASP 的Windows 认证.NET MVC 应用以下的步骤:
...
Windows认证通过在 Windows 域中根据用户帐户验证凭据来验证用户。
