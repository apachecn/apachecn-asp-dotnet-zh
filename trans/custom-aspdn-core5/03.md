...
在大多数项目中，您实际上不需要使用不同的**依赖注入**（**DI**容器。ASP.NET 核心中现有的 DI 实现支持主要的基本功能，并且能够高效快速地工作。但是，其他一些 DI 容器支持一些您可能希望在应用中使用的有趣功能：

*   **Ninject**允许您创建一个支持模块作为轻量级依赖项的应用；例如，您可能希望将模块放入特定目录，并在应用中自动注册它们。
*   您可能希望在应用外部的配置文件中，在 XML 或 JSON 文件中，而不是仅在 C#中配置服务。这是各种 DI 容器中的一个常见功能，但 ASP.NET 核心尚不支持。
