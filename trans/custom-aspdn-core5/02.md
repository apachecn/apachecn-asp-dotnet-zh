...
当您创建一个新的 ASP.NET 核心项目时，您已经配置了`appsettings.json`和`appsettings.Development.json`。您可以也应该使用这些配置文件来配置您的应用；这是预先配置的方式，大多数 ASP.NET 核心开发人员都会寻找一个`appsettings.json`文件来配置应用。这是绝对好的，效果相当好。
