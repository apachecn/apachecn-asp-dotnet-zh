...
您可以使用`Component`标记帮助器将 Razor组件与 MVC 和 Razor 页面集成。执行此操作时，您还可以通过将`render-mode`属性设置为`Static`来预渲染应用（“T1”）组件），从而加快初始渲染时间。预渲染还可用于改进搜索引擎优化（SEO）。“缺点”是需要 ASP.NET Core服务器来执行预渲染逻辑。
...
*   创建和使用 ASP.NET Core剃须刀组件[https://net5.link/iDVh](https://net5.link/iDVh)
