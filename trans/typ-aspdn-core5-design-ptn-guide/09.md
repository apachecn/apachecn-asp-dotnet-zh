...
您不限于`Component`的一个实现和`Composite`的一个实现；根据您的用例，您可以根据需要创建任意数量的`IComponent`实现。然后，您可以混合和匹配它们，使您的数据结构符合您的需要。我们将在[*第 17 章*](17.html#_idTextAnchor325)、*ASP.NET Core用户界面*中探讨如何显示复杂复合。
...
使用依赖注入最困难的部分之一是它的抽象性。如果您不确定所有这些部件是如何组装的，请在 VisualStudio 中添加一个断点（比如在`var result = transparentFacade.ExecuteOperationA()`行），并在调试模式下运行应用。从那里，**进入**每个方法调用。这将有助于你了解发生了什么。使用调试器查找具体类型及其状态有助于查找有关系统的详细信息或诊断错误。
