...
在本章中，我们将介绍 ASP.NET Core的 web API 版本，它是大多数现代技术堆栈的关键部分。此外，我们在本书中使用了这些技术和本章中学习的模式。避免使用用户界面使代码更容易理解。Web API 用于各种类型和规模的项目，从微服务到移动应用。
...
过去，ASP.NET Core使用 JSON.NET 作为底层 JSON 序列化程序。自.NET Core 3.0 以来，他们添加了`System.Text.Json`名称空间，其中包含一个全新的序列化程序。新的序列化程序速度更快，但功能更少。如果您需要 JSON.NET 功能，或者出于兼容性原因，可以通过引用`Microsoft.AspNetCore.Mvc.NewtonsoftJson`NuGet 包来使用它。然后，在`IMvcBuilder`对象上添加对`AddNewtonsoftJson()`扩展方法的调用，如`services.AddControllers().AddNewtonsoftJson();`。
